...
export default function ProfileScreen() {
  ...
  async function fetchGallery() {
    ...
    const { data } = await supabase
      .from('set_media')
      .select(`
        id, storage_path, media_type, label, caption, created_at,
        collection:collection_id ( name, set_number )
      `)
      .eq('user_id', user.id)
      ...
      <div style={styles.content}>
        ...
        <form onSubmit={handleSave} style={styles.form}>
          ...
          <div style={styles.optionRow}>
            <button type="button" style={{ ...styles.optionBtn, ...(publicProfile ? styles.optionBtnActive : {}) }} onClick={() => setPublicProfile(true)}>
              Public
            </button>
            <button type="button" style={{ ...styles.optionBtn, ...(!publicProfile ? styles.optionBtnActive : {}) }} onClick={() => setPublicProfile(false)}>
              Private
            </button>
          </div>
          {publicProfile && profile?.username && (
            <a href={`/u/${profile.username}`} target="_blank" rel="noopener noreferrer" style={styles.profileLinkBtn}>
          ...
          <input style={styles.input} type="text" placeholder="e.g. Louisville, KY" value={location} onChange={e => setLocation(e.target.value)} />

          <p style={styles.sectionLabel}>Bio</p>
          <textarea style={{ ...styles.input, height: '80px', resize: 'none' }} placeholder="Tell other collectors about yourself..." value={bio} onChange={e => setBio(e.target.value)} />
          ...
          <div style={styles.optionRow}>
            {[
              ...
              <button key={opt.value} type="button" style={{ ...styles.optionBtn, ...(collectingGoal === opt.value ? styles.optionBtnActive : {}) }} onClick={() => setCollectingGoal(opt.value)}>
                {opt.label}
              </button>
            ))}
          </div>
          ...
          <div style={styles.optionRow}>
            {[
              ...
              <button key={opt.value} type="button" style={{ ...styles.optionBtn, ...(preferredCondition === opt.value ? styles.optionBtnActive : {}) }} onClick={() => setPreferredCondition(opt.value)}>
                {opt.label}
              </button>
            ))}
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  content: { padding: '24px', maxWidth: '600px', margin: '0 auto', width: '100%', flex: 1 },
  centered: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  title: { fontSize: '28px', fontWeight: 'bold', color: Colors.white, marginBottom: '24px' },
  form: { display: 'flex', flexDirection: 'column', gap: '8px' },
  sectionLabel: { fontSize: '14px', fontWeight: 'bold', color: 'rgba(255,255,255,0.6)', marginTop: '16px', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' },
  staticField: { backgroundColor: 'rgba(0,8,20,0.4)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '14px', fontSize: '16px', color: 'rgba(255,255,255,0.5)' },
  input: { backgroundColor: 'rgba(0,8,20,0.6)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', padding: '14px', fontSize: '16px', color: Colors.white, outline: 'none', width: '100%' },
  optionRow: { display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' },
  optionBtn: { padding: '10px 16px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.2)', backgroundColor: 'rgba(0,8,20,0.6)', color: Colors.white, fontSize: '14px', cursor: 'pointer' },
  optionBtnActive: { backgroundColor: Colors.yellow, borderColor: Colors.yellow, color: Colors.text.onYellow, fontWeight: 'bold' },
  themeGrid: { display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' },
  themeBtn: { padding: '8px 14px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.2)', backgroundColor: 'rgba(0,8,20,0.6)', color: Colors.white, fontSize: '13px', cursor: 'pointer' },
  themeBtnActive: { backgroundColor: Colors.yellow, borderColor: Colors.yellow, color: Colors.text.onYellow, fontWeight: 'bold' },
  saveBtn: { backgroundColor: Colors.yellow, color: Colors.text.onYellow, border: 'none', borderRadius: '8px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', marginTop: '24px' },
  collectionBtn: { width: '100%', padding: '16px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', backgroundColor: 'rgba(0,8,20,0.6)', color: Colors.white, fontSize: '16px', cursor: 'pointer', marginTop: '12px' },
  backBtn: { background: 'none', border: 'none', color: Colors.yellow, fontSize: '16px', cursor: 'pointer', marginBottom: '8px', padding: 0 },
  profileLinkBtn: { display: 'block', color: Colors.yellow, fontSize: '14px', textDecoration: 'none', marginTop: '8px' },
  galleryCard: { backgroundColor: 'rgba(0,8,20,0.6)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '12px', padding: '20px', marginBottom: '24px' },
  chartTitle: { fontSize: '16px', fontWeight: 'bold', color: Colors.white, marginBottom: '16px', textAlign: 'center' },
  galleryGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' },
  galleryItem: { backgroundColor: 'rgba(0,8,20,0.4)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', overflow: 'hidden' },
  galleryImg: { width: '100%', height: '140px', objectFit: 'cover' as const, cursor: 'pointer', display: 'block' },
  galleryInfo: { padding: '8px' },
  gallerySetName: { fontSize: '12px', fontWeight: 'bold', color: Colors.white, marginBottom: '4px', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' },
  galleryLabel: { backgroundColor: 'rgba(251,224,45,0.15)', color: Colors.yellow, fontSize: '10px', fontWeight: 'bold', padding: '2px 6px', borderRadius: '10px', textTransform: 'capitalize' as const },
  galleryCaption: { fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginTop: '4px' },
